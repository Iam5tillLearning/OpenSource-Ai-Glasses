...
本目录同时包含 BLE SDK demo 的眼镜端和外部客户端示例：

- 眼镜端：`glasses/ble_demo.c`，通过 `ai_glass_sdk` 订阅 `sdk.demo.ping` 并回发 `sdk.demo.pong`。
- Android 客户端：`clients/android/`，通过系统 BLE API 连接眼镜，发送测试指令并展示 notify 回包。
- Windows 客户端：`clients/windows/`，保留 Windows BLE GATT client 接入口径。
...
2. 打开 Android 设备蓝牙；部分机型还需要开启系统定位服务才能扫描 BLE。
3. 点击 `Scan OSAIG`。
...
Windows 端应作为 BLE GATT client 连接眼镜暴露的 Service UUID `00001910-0000-1000-8000-00805f9b34fb` 与 Characteristic UUID `dfd4416e-1810-47f7-8248-eb8be3dc47f9`。写入方向使用 UTF-8 JSON，回包通过 notify 接收；协议字段和包长限制见根项目 `docs/api/ai_ble_protocol.md`。

## 6. 故障排查

- 扫描不到设备：确认眼镜 BLE 名称为 `OSAIG-XXXX`，并检查客户端蓝牙权限和定位服务。
